...
export class PutBlobContext {
    ...
    createWriteStream(): streams.Writable {
        ...
        const dstream = this._stream = new WithDigestStream(this._digestAlgorithm);
        dstream.on('end', () => {
...
export function preparePutBlob(uploadId: string, name: string, digest: string): Promise<PutBlobContext> {
    try {
